...
from memory_db import TotalRecallError
...
class TestEmbeddingErrors:
    ...
    def test_missing_api_key_raises_error(self, monkeypatch):
        """Missing API key raises TotalRecallError with helpful message."""
        ...
        # Should raise TotalRecallError, not generic ValueError
        with pytest.raises(TotalRecallError) as exc_info:
            get_embedding("test for missing api key", use_cache=False)

        assert "OPENAI_TOKEN_TOTAL_RECALL_EMBEDDINGS" in str(exc_info.value)
        assert exc_info.value.error_code == "missing_api_key"

    def test_api_call_failure_wrapped(self, monkeypatch):
        """API failures are wrapped in TotalRecallError."""
...
class TestDatabaseErrors:
    ...
    def test_database_init_failure_wrapped(self, tmp_path, monkeypatch):
        """Database init failures are wrapped in TotalRecallError."""
        import memory_db
    ...
    def test_corrupted_database_handled(self, tmp_path, monkeypatch):
        ...
        # Test that at least the connection succeeds or we get TotalRecallError
        try:
            db = memory_db.get_db()
...
class TestIndexerErrors:
    ...
    def test_missing_transcript_handled(self, tmp_path, monkeypatch):
        """Missing transcript file raises TotalRecallError."""
        import memory_db
...
class TestTotalRecallError:
    """Test TotalRecallError exception class."""

    def test_error_has_code_and_message(self):
        """TotalRecallError has error code and message."""
        error = TotalRecallError("Something went wrong", "test_error")
        assert str(error) == "Something went wrong"
        assert error.error_code == "test_error"

    def test_error_has_optional_details(self):
        """TotalRecallError can include additional details."""
        error = TotalRecallError(
            "API failed",
            "api_error",
    ...
    def test_error_is_json_serializable(self):
        """TotalRecallError can be serialized to JSON."""
        import json

        error = TotalRecallError("Test error", "test_code", details={"key": "value"})
