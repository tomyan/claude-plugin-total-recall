...
def _migrate_schema(db):
    ...
    if "start_time" not in span_columns:
        logger.info("Adding start_time column to spans table")
        db.execute("ALTER TABLE spans ADD COLUMN start_time TEXT")
        db.commit()

    if "end_time" not in span_columns:
        logger.info("Adding end_time column to spans table")
    ...
    if "first_seen" not in topic_columns:
        logger.info("Adding first_seen column to topics table")
        db.execute("ALTER TABLE topics ADD COLUMN first_seen TEXT")
        db.commit()

    if "last_seen" not in topic_columns:
        logger.info("Adding last_seen column to topics table")
    ...
    if "message_time" not in idea_columns:
        logger.info("Adding message_time column to ideas table")
...
def migrate_timestamps_from_transcripts() -> dict:
    ...
    logger.info(f"Timestamp migration: {len(files_to_process)} files, {ideas_updated} ideas, {spans_updated} spans")
    return {
